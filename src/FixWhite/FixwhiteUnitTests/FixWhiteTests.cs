...
namespace FixwhiteUnitTests
{
    ...
    public class FixWhiteTests
    {
        ...
        public void TestTabsWithNoChange()
        {
            ...
            Assert.AreEqual(0, CallMain("", sourceFile.FullName, ResultsFile.FullName));
        ...
        [TestMethod]
        public void TestOverwriteNoChange()
        {
            string inputString = $"\t   This is    a test\t";
            string expected = inputString;
            FileInfo sourceFile = CreateFile(SourceFile, inputString);
            FileInfo comparisonFile = CreateFile("Comparison " + SourceFile, inputString);
        ...
        public void AsciiYesChange()
        {
            ...
            AppendFile(fi, $"This is backwards: {LineFeedCarriageReturn}");
            Assert.AreEqual(0, CallMain("", fi.FullName, ResultsFile.FullName));
        ...
        {
            string fileName = "UnicodeYesChange.txt";
            FileInfo fi = CreateFile(fileName, $"This is a test:{Environment.NewLine}", Encoding.Unicode);
            ...
            AppendFile(fi, $"This is backwards: {LineFeedCarriageReturn}", Encoding.Unicode);
            Assert.AreEqual(0, CallMain("", fi.FullName, ResultsFile.FullName));
            string comparisonFileName = Path.Combine(ComparisonDirectory.FullName, fileName);
            AssertFilesEqual(ResultsFile.FullName, comparisonFileName);
        }
...
/*
 * 
 *                 writer.Write($"This is a test{whiteSpace}");
                writer.Write($"This was only a test{whiteSpace}");
                writer.Write($"{whiteSpace}");
                writer.Write($"        {whiteSpace}");
                writer.Write($"{whiteSpace}                 ");
                writer.Write($"     {whiteSpace}                           ");
                writer.Write($"    This is a test of spaces at the begining{whiteSpace}");
                writer.Write($"This was a test of spaces at the end            {whiteSpace}");
                writer.Write($"\tThis is a test of tab at the begining{whiteSpace}");
                writer.Write($"This was a test of spaces at the end\t\t{whiteSpace}");
                writer.Write($"No CRLF");
